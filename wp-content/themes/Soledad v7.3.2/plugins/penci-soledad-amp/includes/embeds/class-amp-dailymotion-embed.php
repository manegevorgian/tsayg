...
class Penci_AMP_DailyMotion_Embed_Handler extends Penci_AMP_Base_Embed_Handler {
	...
	public function render( $args ) {
		...
		if ( empty( $args['video_id'] ) ) {
			return Penci_AMP_HTML_Utils::build_tag( 'a', array( 'href' => esc_url( $args['url'] ), 'class' => 'amp-wp-embed-fallback' ), esc_html( $args['url'] ) );
		}

		$this->did_convert_elements = true;

		return Penci_AMP_HTML_Utils::build_tag(
			'amp-dailymotion',
			array(
	...
	private function get_video_id_from_url( $url ) {
		$parsed_url = Penci_AMP_WP_Utils::parse_url( $url );
