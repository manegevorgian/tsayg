...
class Penci_AMP_Analytics_Options_Submenu_Page {
	...
	private function render_entry( $id = '', $type = '', $config = '' ) {
		...
		if ( $is_existing_entry ) {
			...
			$analytics_title = sprintf( __( 'Analytics: %s', 'penci-amp' ), $entry_slug );
		} else {
			$analytics_title = __( 'Add new entry:', 'penci-amp' );
		}
		?>
		<div class="analytics-data-container">
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<div class="options">
					...
					<p>
						<label>
							<?php echo __( 'JSON Configuration:', 'penci-amp' ) ?>
							<br />
							<textarea rows="10" cols="100" name="config"><?php echo esc_textarea( $config ); ?></textarea>
						</label>
					</p>
					<input type="hidden" name="action" value="penci_amp_analytics_options">
				</div>
				<p>
					<?php
					wp_nonce_field( 'analytics-options', 'analytics-options' );
					submit_button( __( 'Save', 'penci-amp' ), 'primary', 'save', false );
					if ( $is_existing_entry ) {
						submit_button( __( 'Delete', 'penci-amp' ), 'delete button-primary', 'delete', false );
					}
					?>
				</p>
			</form>
	...
	public function render_title() {
		...
		if ( isset( $_GET['valid'] ) ) {
			...
			if ( $is_valid ) {
				$admin_notice_text = __( 'The analytics entry was successfully saved!', 'penci-amp' );
				$admin_notice_type = 'success';
			} else {
				$admin_notice_text = __( 'Failed to save the analytics entry. Please make sure that the JSON configuration is valid and unique.', 'penci-amp' );
			...
			if ( $admin_notice_text ) : ?>
				<div class="amp-analytics-options notice <?php echo esc_attr( 'notice-' . $admin_notice_type ); ?> is-dismissible">
					...
					<button type="button" class="notice-dismiss">
						<span class="screen-reader-text"><?php __( 'Dismiss this notice.', 'penci-amp' ) ?></span>
					</button>
				</div>
			<?php endif; ?>
		</div><!-- .wrap -->
