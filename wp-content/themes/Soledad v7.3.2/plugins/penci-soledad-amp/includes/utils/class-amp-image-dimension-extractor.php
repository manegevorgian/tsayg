...
class AMP_Image_Dimension_Extractor {
	...
	static public function extract( $urls ) {
		...
		$extracted_dimensions = apply_filters( 'penci_amp_extract_image_dimensions_batch', $extracted_dimensions );
	...
	public static function normalize_url( $url ) {
		...
		$parsed = Penci_AMP_WP_Utils::parse_url( $url );
	...
	private static function register_callbacks() {
		...
		add_filter( 'penci_amp_extract_image_dimensions_batch', array( __CLASS__, 'extract_by_downloading_images' ), 999, 1 );

		do_action( 'penci_amp_extract_image_dimensions_batch_callbacks_registered' );
	}
	...
	private static function determine_which_images_to_fetch( &$dimensions, &$urls_to_fetch ) {
		foreach ( $dimensions as $url => $value ) {
			...
			$url_hash = md5( $url );
			$transient_name = sprintf( 'penci_amp_img_%s', $url_hash );
			...
			$transient_lock_name = sprintf( 'penci_amp_lock_%s', $url_hash );
	...
	private static function fetch_images_via_fast_image( $urls_to_fetch, &$images ) {
		if ( ! class_exists( 'FastImage' ) ) {
			require_once( PENCI_AMP_DIR. '/includes/lib/fastimage/class-fastimage.php' );
		}
	...
	private static function fetch_images_via_faster_image( $urls_to_fetch, &$images ) {
		...
		if ( ! function_exists( 'penci_amp_get_fasterimage_client' ) ) {
			require_once( PENCI_AMP_DIR. '/includes/lib/fasterimage/amp-fasterimage.php' );
		}

		$user_agent = apply_filters( 'penci_amp_extract_image_dimensions_get_user_agent', self::get_default_user_agent() );
		$client = penci_amp_get_fasterimage_client( $user_agent );
	...
	public static function get_default_user_agent() {
		return 'amp-wp, v' . PENCI_AMP_VERSION . ', ' . get_site_url();
	}
}
