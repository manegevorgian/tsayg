...
<?php
    ...
    function example_ajax_request() {
        if ( isset($_REQUEST) ) {
            ...
            $q_ans=$wpdb->get_results("SELECT `answer` FROM wp_tco_poll_answers WHERE `question_id`='$q_id'",ARRAY_A)  ;;
            echo json_encode([data => "<input type='text' value='1' />"]);
        }
        die();
    }
...
//    add_action( 'nopriv_'.$_POST['action'], 'my_ajax' );
//    add_action( $_POST['action'], 'my_ajax' );
//    function my_ajax() {
//        echo 555;
//        die();  // Required for a proper Wordpress AJAX result
//    }
    //    $question1 = $poll[0]['question'];
//    $answers1 = explode('/', $poll[0]['answers']);
//    var_dump($poll[0]['answers']);
//
//    if($_POST["action"]=='delete')
//    {
//        $wpdb->delete( $tableName, array( 'question' => $question1 ) );
//    }
//]
    
//    function show_inputs($request) {
//        var_dump($request);
//        return 1;
//    }
?>
<div class="container row" id="pollOptions">
            ...
            <table class= "table">
                ...
                <?php foreach($questions as $q){ ?>
                    <tr>
                        ...
                        <td scope="col">
                            <button type="button" class=" btn btn-outline-success btn-sm mr-0   show " data-quest="<?=$q["id"] ?>">View</button>
                            <button  type="button" class=" btn btn-outline-danger btn-sm mr-0  delete"  data-quest="<?=$q["id"] ?>">Delete</button>
                        </td>
                    </tr>
                <?php } ?>
                </tbody>
    ...
    <div class="container col-6 modal-show">
        <?php foreach($q_ans as $ans) {?>
...
<script type='text/javascript'>
    window.addEventListener('load',function () {
        ...
        let btn_delete = jQuery(".delete");
        let btn_show = jQuery(".show");
